...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if isinstance(self, ContainerAnt) and not isinstance(place.ant, ContainerAnt):
                self.contain_ant(place.ant)
                place.ant = self
                Insect.add_to(self, place)
                return
            if isinstance(place.ant, ContainerAnt):
                assert place.ant.can_contain(self) == True, 'Two ants in {0}'.format(place)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while count < min_range:
            p = p.entrance
            if p is beehive or p is None:
                return None
            count += 1
        while p is not beehive and count <= max_range:
            if p.bees:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in p.bees[:]:
                bee.reduce_armor(self.damage + min(amount, armor))
        else:
            for bee in p.bees[:]:
                bee.reduce_armor(min(amount, armor))
        # END Problem 5
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        Place.add_insect(self=self, insect= insect)
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.is_exsiting:
            self.true_queen = False
        else:
            QueenAnt.is_exsiting = True
        # END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.true_queen:
            ...
            p = self.place
            while p is not None:
                if p.ant and p.ant != self and not hasattr(p.ant, 'doubled'):
                    p.ant.damage *= 2
                    p.ant.doubled = True
                p = p.exit
        else:
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 5
        # print("DEBUG:", distance, self.insects_shot)
        return max(0, 2 - 0.2 * distance - 0.05 * self.insects_shot)
        # END Problem Optional 5
