...
def subseqs(s):
    ...
    if s == []:
        ...
        seqs = insert_into_all(s[0], subseqs(s[1:]))
        return seqs + subseqs(s[1:])
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            ...
            a = subseq_helper(s[1:], prev)  # choose not to include s[0]
            b = subseq_helper(s[1:], s[0])  # choose to include s[0]
            return a + insert_into_all(s[0], subseq_helper(s[1:], s[0]))
            # back to the subseqs functio
            # seqs = insert_into_all(s[0], subseqs(s[1:]))
            # return seqs + subseqs(s[1:])n
            # seqs : choose not to include s[0]
            # insert_into_all(s[0], subseqs(s[1:])) : choose to include s[0]
            # it is a binary tree, 2 situations: choose or not choose
