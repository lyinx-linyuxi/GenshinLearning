...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    given_city = make_city('given_city', lat, lon)
    if distance(given_city, city_a) < distance(given_city, city_b):
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        branche = []
        for leaf in leaves:
            branche.append(tree(leaf))
        t = tree(label(t), branche)
        return t
    if is_tree(t):
        branche = []
        for branch in branches(t):
            branche.append(sprout_leaves(branch, leaves)) 
        t = tree(label(t), branche)
        return t
...
def add_trees(t1, t2):
    ...
    if is_tree(t1) and is_tree(t2):
        bs = [add_trees(branch1, branch2) for branch1, branch2 in zip(branches(t1), branches(t2))]
        if len(branches(t1)) > len(branches(t2)):
            bs += branches(t1)[len(branches(t2)):]
        if len(branches(t1)) < len(branches(t2)):
            bs += branches(t2)[len(branches(t1)):]
