...
def is_bst(t):
    ...
    if t.is_leaf():
        ...
        if len(t.branches) == 1:
            ...
            return is_bst(t.branches[0]) and is_bst(t.branches[1]) and max_tree(t.branches[0]) <= t.label and min_tree(t.branches[1]) >= t.label
        else:
            return False

def max_tree(t):
    """Returns the max of a Tree t."""
...
def min_tree(t):
    """Returns the min of a Tree t."""

    if t.is_leaf():
        return t.label
    else:
        return min([t.label] + [min_tree(b) for b in t.branches])
