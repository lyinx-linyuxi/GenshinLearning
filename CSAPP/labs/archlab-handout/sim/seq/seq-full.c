...
long long gen_instr_valid()
{
    return ((icode) == (I_NOP) || (icode) == (I_HALT) || (icode) == 
      (I_RRMOVQ) || (icode) == (I_IRMOVQ) || (icode) == (I_IADDQ) || 
      (icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ) || (icode) == (I_ALU)
...
long long gen_need_regids()
{
    return ((icode) == (I_RRMOVQ) || (icode) == (I_ALU) || (icode) == 
      (I_PUSHQ) || (icode) == (I_POPQ) || (icode) == (I_IRMOVQ) || (icode)
       == (I_IADDQ) || (icode) == (I_RMMOVQ) || (icode) == (I_MRMOVQ));
}

long long gen_need_valC()
{
    return ((icode) == (I_IRMOVQ) || (icode) == (I_RMMOVQ) || (icode) == 
      (I_IADDQ) || (icode) == (I_MRMOVQ) || (icode) == (I_JMP) || (icode)
       == (I_CALL));
}
...
long long gen_srcB()
{
    return (((icode) == (I_ALU) || (icode) == (I_RMMOVQ) || (icode) == 
        (I_IADDQ) || (icode) == (I_MRMOVQ)) ? (rb) : ((icode) == (I_PUSHQ)
...
long long gen_aluA()
{
    return (((icode) == (I_RRMOVQ) || (icode) == (I_ALU)) ? (vala) : (
        (icode) == (I_IRMOVQ) || (icode) == (I_IADDQ) || (icode) == 
...
long long gen_aluB()
{
    return (((icode) == (I_RMMOVQ) || (icode) == (I_IADDQ) || (icode) == 
        (I_MRMOVQ) || (icode) == (I_ALU) || (icode) == (I_CALL) || (icode)
