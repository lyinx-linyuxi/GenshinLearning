...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.var_name in env:
            ...
            # raise LookupError("Variable no defined.")
            return None
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        # if self.operator not in env.keys():
        #     e = LookupError("CallExpression not defined.")
        #     raise e
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        arg_names = []
        if isinstance(self.body, LambdaExpr):
            arg_names = self.body.parameters
        elif isinstance(self.body, CallExpr):
            arg_names = [x.var_name for x in self.body.operands if isinstance(x, Name)]
        for x, y in zip(arg_names, arguments):
                new_env[x] = y
