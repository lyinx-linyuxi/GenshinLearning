...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        # BEGIN PROBLEM 3
        "*** YOUR CODE HERE ***"
        def convert(scheme_list):
            if scheme_list == nil:
                return
            else:
                python_args.append(scheme_list.first)
                convert(scheme_list.rest)
        convert(args)
        if self.use_env:
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        lam = do_lambda_form(Pair(target.rest, expressions.rest), env)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        name, val = bindings.first.first, bindings.first.rest.first
        validate_formals(Pair(name, nil))
...
def do_define_macro(expressions, env):
    ...
    if not scheme_listp(expressions.first):
        raise SchemeError('badly formed expression: ' + repl_str(expressions))
    name = expressions.first.first
    if not scheme_symbolp(name):
        raise SchemeError('badly formed expression: ' + repl_str(expressions))
    macro = MacroProcedure(expressions.first.rest, expressions.rest, env)
...
class MuProcedure(Procedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        for _ in range(len(self.formals)):
            env.define(self.formals.first, args.first)
            self.formals, args = self.formals.rest, args.rest
        return env
    # END PROBLEM 18
